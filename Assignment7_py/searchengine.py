                ...
                for word in words:
                    if word!="":
                        if word.lower() in index.keys():
                            if filename not in index[word.lower()]:
                                index[word.lower()].append(filename)
                        else:
                            index[word.lower()]=[filename]
                line_num += 1
...
def search(index, query):
    ...
    for word in words:
        tmp_result=[]
        if str(word) in index.keys():
            for filename in index[word]:
                tmp_result.append(filename)

        if i==0:
            result=tmp_result
        else:
            #contain *all* of the terms in the query (using the index passed in).
            list1_as_set = set(result)
            intersection = list1_as_set.intersection(tmp_result)
