...
def check_move(grid, x1, y1, x2, y2):
    ...
    """
    if x2 < 0 or x2 >= len(grid[0]) or y2 < 0 or y2 >= len(grid):
        return False
    elif x1==x2 or y1==y2:
        if grid[y2][x2]==None:
            return True
...
def do_gravity(grid, x, y):
    ...
    # at bottom already
        ...
        return grid
    # down
    elif grid[y+1][x]==None and check_move(grid, x, y, x, y+1):
        do_move(grid, x, y, x, y+1)   
        return grid   
    # x is on the left
    elif x==0:
        # bottom blocked - not check down is empty
    ...
    # x is in the middle            
        ...
        # bottom blocked - not check down is empty
        if grid[y+1][x-1]!=None and grid[y+1][x+1]!=None:  
          return grid  
         # down-left
        elif grid[y+1][x-1]==None and check_move(grid, x, y, x-1, y+1):
            do_move(grid, x, y, x-1, y+1)   
            return grid
        # down-right
        else:
...
def do_brownian(grid, x, y, brownian):
    ...
    if check_move(grid, x, y, x, y+1):
        ...
        if num < brownian:
            ...
            if coin==0: # move one cell to the left.
                if check_move(grid, x, y, x-1, y):
                    do_move(grid, x, y, x-1, y)   
            else:      # move one cell to the right.
                if check_move(grid, x, y, x+1, y):
                    do_move(grid, x, y, x+1, y)   
    return grid
    ...
    for row in reversed(range(len(grid))):
        for column in range(len(grid[0])):
            if grid[row][column]=='s':
                do_brownian(grid,column,row,brownian)
            do_gravity(grid, column, row)
