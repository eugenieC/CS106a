...
def load_data(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            # print(s)
            city_name=s[0].strip()
            # insert the city name with day 1 number
            cumulative_cases[city_name]=[int(s[1].strip())]
            # append day 2 to day 7
            for i in range(2,len(s)):
                cumulative_cases[city_name].append(int(s[i].strip()))
...
def daily_cases(cumulative):
    ...
    for k, v in cumulative.items():
        # put key with day 1 number
        daily[k]=[v[0]]
        for j in range(1,len(v)):
            # append daily increase
            daily[k].append(v[j] - v[j-1])
...
def main():
    filename = 'Assignment6/disease1.txt'
